...
class H2hProductController extends Controller
{
    /**
     * Display raw synchronized H2H products list, sync stats, and execution logs.
     */
    public function index(Request $request): Response
    {
        ...
        $categories = H2hProduct::distinct()->orderBy('category')->pluck('category');
        $brands = H2hProduct::distinct()->orderBy('brand')->pluck('brand');
        $recentLogs = ProductSyncLog::latest()->take(30)->get();

        return Inertia::render('Admin/Products/H2hSync', [
            'products' => $products,
            'filters' => [
                'search' => $search ?? '',
